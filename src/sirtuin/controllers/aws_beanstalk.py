...
def _get_environment_variables(
    ...
    return read_dotenv_file(f"{config.directory}/{config.beanstalk.dotenv_path}")


def _write_ebignore(config: ElasticBeanstalkSirtuinConfig) -> str:
    filepath = f"{config.directory}/{DEFAULT_BEANSTALK_EBIGNORE_PATH}"

    dump_as_raw(f"*\n\n!{DEFAULT_BEANSTALK_ARTIFACT_PATH}", filepath)
...
def _write_beanstalk_config(config: ElasticBeanstalkSirtuinConfig) -> str:
    ...
    filepath = f"{config.directory}/{DEFAULT_BEANSTALK_CONFIG_PATH}"
...
def _write_dockerrun_config(config: ElasticBeanstalkSirtuinConfig) -> str:
    ...
    filepath = f"{config.directory}/{DEFAULT_BEANSTALK_DOCKERRUN_PATH}"
...
def _write_extensions(
    ...
    for file, variables in files.items():
        filepath = f"{target_directory}/{file}"

        copy_file(f"{schema_source}/{file}", filepath)
...
def _write_beanstalk_customization(config: ElasticBeanstalkSirtuinConfig) -> list[str]:
    ...
    _write_extensions(
        config.ebextensions,
        DEFAULT_BEANSTALK_EBEXTENSIONS_SCHEMAS,
        f"{config.directory}/{DEFAULT_BEANSTALK_EBEXTENSIONS_DIRECTORY}",
        extensions,
    )
    ...
    _write_extensions(
        config.platform,
        DEFAULT_BEANSTALK_PLATFORM_SCHEMAS,
        f"{config.directory}/{DEFAULT_BEANSTALK_PLATFORM_DIRECTORY}",
        extensions,
    )
...
def _setup_beanstalk_deployment(config: ElasticBeanstalkSirtuinConfig) -> str:
    filepaths: list[str] = [
        _write_ebignore(config),
        _write_beanstalk_config(config),
        _write_dockerrun_config(config),
        *_write_beanstalk_customization(config),
    ]

    artifact = f"{config.directory}/{DEFAULT_BEANSTALK_ARTIFACT_PATH}"

    with ZipFile(artifact, mode="w") as zip_file:
        for filepath in filepaths:
            zip_file.write(filepath, arcname=filepath.replace(config.directory, "."))

    return artifact


def _clean_beanstalk_deployment(config: ElasticBeanstalkSirtuinConfig) -> None:
    delete_file(f"{config.directory}/{DEFAULT_BEANSTALK_ARTIFACT_PATH}")
    delete_file(f"{config.directory}/{DEFAULT_BEANSTALK_EBIGNORE_PATH}")
    delete_file(f"{config.directory}/{DEFAULT_BEANSTALK_DOCKERRUN_PATH}")

    delete_directory(f"{config.directory}/{DEFAULT_BEANSTALK_CONFIG_DIRECTORY}")
    delete_directory(f"{config.directory}/{DEFAULT_BEANSTALK_EBEXTENSIONS_DIRECTORY}")
    delete_directory(f"{config.directory}/{DEFAULT_BEANSTALK_PLATFORM_DIRECTORY}")
